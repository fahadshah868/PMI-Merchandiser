...
namespace TradeTrackerProject.ConstantUserCode
{
    ...
    public class UserCodeCollection
    {
    	...
    	public static void visitNonTKAllTPOSM()
    	{
    		...
    		for(int i=0; i<tposmlist.Count; i++){
    			...
    			if(tposmtext.Contains("Key Message Communication") || tposmtext.Contains("Trade Letter")){
    				TradeTrackerProject.TestCases.Non_TK_ShopOpen.VisitTPOSM1.Start();
    			}
    			else{
    				TradeTrackerProject.TestCases.Non_TK_ShopOpen.VisitTPOSM2.Start();
    			}
    		}
    		try{
    			while(true){
    				var lastitemcontainerbeforeswipe = repo.PkComTradetracker.List4.TPOSMList.Children.Last();
    				var lastitembeforeswipe = lastitemcontainerbeforeswipe.Children.ToList();
    				string lastitemtextbeforeswipe = Convert.ToString(lastitembeforeswipe[1]);
	    			...
	    			Delay.Milliseconds(2000);
		            var lastitemcontainerafterswipe = repo.PkComTradetracker.List4.TPOSMList.Children.Last();
		            var lastitemafterswipe = lastitemcontainerafterswipe.Children.ToList();
    				string lastitemtextafterswipe = Convert.ToString(lastitemafterswipe[1]);
    				if(lastitembeforeswipe == lastitemafterswipe){
		            	break;
		            }
		            else{
    					...
    					Delay.Milliseconds(500);
    		 			if(lastitemtextafterswipe.Contains("Key Message Communication") || lastitemtextafterswipe.Contains("Trade Letter")){
		    				TradeTrackerProject.TestCases.Non_TK_ShopOpen.VisitTPOSM1.Start();
		    			}
		    			else{
		    				TradeTrackerProject.TestCases.Non_TK_ShopOpen.VisitTPOSM2.Start();
    	...
    	/// </summary>
    	[UserCodeMethod]
    	public static void visitTKAllTPOSM()
    	{
    		var tposmlist = repo.PkComTradetracker.List4.TPOSMList.Children.ToList();
    		for(int i=0; i<tposmlist.Count; i++){
    			var tposm = tposmlist[i].Children.ToList();
    			...
    			Delay.Milliseconds(500);
    			if(tposmtext.Contains("Key Message Communication") || tposmtext.Contains("Trade Letter")){
    				TradeTrackerProject.TestCases.TK_ShopOpen.VisitTPOSM1.Start();
    			}
    			else{
    				TradeTrackerProject.TestCases.TK_ShopOpen.VisitTPOSM2.Start();
    			}
    		}
    		try{
    			while(true){
    	...
    	{
    		int countlist = repo.PkComTradetracker.ShopOptions_ListView.Children.Count;
    		if(countlist == 2){
